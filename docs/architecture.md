...
flowchart TB
    ...
    Entry --> Out["Consumer<br/>embedder code"]
...
Every provider produces the same normalized events, so the consumer (embedder loop, logger, HTTP/SSE adapter) doesn't need to know the backend.

```mermaid
sequenceDiagram
    participant C as Consumer (embedder)
...
- **Consumers stay simple**: any embedder loop reads one event type and doesn't care whether we're talking to a local Ollama server, a cloud API, or a full coding-agent CLI subprocess.
- **Swappable backends**: pick a `Provider` at construction time — the `Agent` class is identical for all of them.
